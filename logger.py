...
def setup_logger(cfg):
    ...
    formatter = MyFormatter(fmt='[%(asctime)s|%(threadName)12s|%(levelname)8s] %(message)s',datefmt='%Y-%m-%dT%H:%M:%S.%fZ')
    # formatter = MyFormatter(fmt='[%(asctime)s | {:11s} | %(threadName)13s | %(levelname)8s] %(message)s'.format(name),
    ...
    log.info('{:s} Logger Initialized'.format(name))
    return log
